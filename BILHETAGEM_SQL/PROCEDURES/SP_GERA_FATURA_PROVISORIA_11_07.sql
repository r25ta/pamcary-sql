...
/*RETIRAR APOS AJUSTE NA PROCEDURE SP_GERAR_TAXA_LIMPEZA_PISTA*/
...
INSERT
   ...
   seq_numer_fat)
VALUES ('xx',
'SQLEXCEPTION - vai abrir bilhet.sp_estorno_procs_fatura - ' CONCAT v_err_message,
CURRENT_TIMESTAMP,
...
CALL bilhet.sp_estorno_procs_fatura (p_ctl_contr_taf,
v_msg,
v_err_msg);
...
INSERT
   ...
   seq_numer_fat)
VALUES ('xx',
'SQLEXCEPTION - vai abrir gedad.sp_dad_finaliza_tarefa_conco - ' CONCAT v_err_message,
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
CALL gedad.sp_dad_finaliza_tarefa_conco(p_ctl_contr_taf,
v_msg,
v_err_msg);
...
CREATE INDEX SESSION.idx_cursor_proposta ON
SESSION.cursor_proposta (seq_numer_prs);
...
INSERT
   ...
   num_grupo_seq,
   seq_gera_fat)
VALUES (to_char(v_step),
'parametros - p_competencia: ' || p_competencia || ' p_ctl_contr_taf:' || p_ctl_contr_taf || ' p_num_perio_fat:' || p_num_perio_fat || ' p_cod_user:' || p_cod_user ,
current_timestamp,
NEXT value FOR bilhet.seq,
v_grupo_seq,
0);
...
--QUINZENAL
 ELSEIF (p_num_perio_fat = 2) THEN
--2a QUINZENA
    SET v_num_perio = 2;

--QUINZENAL
 ELSEIF (p_num_perio_fat = 3) THEN
--MENSAL
    SET v_num_perio = 3;
...
INSERT
   INTO    session.cursor_proposta (seq_numer_prs,
   num_dia_vec,
   ano_mes)
 SELECT
   DISTINCT seq_numer_prs,
   num_dia_vec,
   ano_mes
FROM    (
   SELECT
       a.seq_numer_prs,
       num_dia_vec,
       YEAR(b.dhr_embar) || RIGHT('0' || MONTH(b.dhr_embar), 2) AS ano_mes
   ...
   UNION
       SELECT
           aa.seq_numer_prs,
           num_dia_vec,
           YEAR(bb.dhr_embar) || RIGHT('0' || MONTH(bb.dhr_embar), 2) AS ano_mes
       ...
       UNION
           SELECT
               aaa.seq_numer_prs,
               num_dia_vec,
               YEAR(bbb.dhr_embar) || RIGHT('0' || MONTH(bbb.dhr_embar), 2) AS ano_mes
...
INSERT
   ...
   seq_numer_prs)
VALUES (to_char(v_step),
'v_seq_numer_prs - ' || v_seq_numer_prs,
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
SET v_dat_venci = bilhet.fc_bil_data_vencimento (v_seq_numer_prs,
p_num_perio_fat,
v_comp,
'FATURA');
...
SET v_seq_numer_fat = (
SELECT
   COALESCE(seq_numer_fat, 0)
FROM    FINAL TABLE (
   INSERT
       INTO            bilhet.tbil_fatura (dhr_fatur,
           seq_numer_prs,
           dat_venci_fat,
           num_mes_cpt,
           sit_fatur,
...
CALL bilhet.sp_gera_fatura_filho(v_seq_numer_prs,
v_seq_numer_fat,
v_data_procs,
v_dat_venci,
v_num_mes_cpt,
...
IF nvl(v_seq_numer_fat_filho, 0) > 0 THEN 
   ...
   seq_numer_fat)
VALUES (to_char(v_step),
'gerou fatura filho seq_numer_fat_filho - ' || v_seq_numer_fat_filho,
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
CALL bilhet.sp_atualiza_visao_propt ( v_seq_numer_prs,
p_cod_user,
v_msg,
v_err_msg);
...
INSERT
   ...
   seq_numer_fat)
VALUES (to_char(v_step),
'vai abrir tbil_valor_soma_tipo_tarifa: seq_numer_prs - ' || v_seq_numer_prs || ' seq_numer_fat - ' || v_seq_numer_fat || ' VIGÊNCIA: v_dat_inici_vig: ' || TO_CHAR(v_dat_inici_vig, 'DD/MM/YYYY')|| ' v_dat_fim_vig: ' || TO_CHAR(v_dat_fim_vig, 'DD/MM/YYYY'),
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
--INSERE TARIFAS COM VALORES DE: COMERCIAL DA PROPOSTA (NÃO PERCURSO) + COMERCIAL DA PROPOSTA (EM PERCURSO) + MERCADO (EM PERCURSO) EXCEPT COMERCIAL DA PROPOSTA (EM PERCURSO)
    INSERT
   INTO    bilhet.tbil_valor_soma_tipo_tarifa (seq_numer_fat,
   num_tarif,
   idt_premi,
   vlr_calcu,
   dhr_alter,
...
CALL bilhet.sp_gera_premio_base (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
v_seq_numer_fat_filho,
p_cod_user,
...
INSERT
   ...
   seq_numer_fat)
VALUES (to_char(v_step),
'vai abrir bilhet.sp_gera_premio_tarifa - v_seq_numer_fat: ' || v_seq_numer_fat,
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
CALL bilhet.sp_gera_premio_tarifa (v_seq_numer_fat,
p_cod_user,
v_msg,
v_err_msg);
...
CALL bilhet.sp_gera_taxa_pamtax (v_dat_inici_vig,
v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
p_cod_user,
...
CALL bilhet.sp_gera_premio_minimo (v_dat_inici_vig,
v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
p_cod_user,
...
CALL bilhet.sp_gera_desconto_comercial (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
v_seq_numer_fat_filho,
p_cod_user,
...
CALL bilhet.sp_gera_premio_comercial (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
v_seq_numer_fat_filho,
p_cod_user,
...
CALL bilhet.sp_gera_credito_debito (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
p_cod_user,
v_msg,
...
CALL bilhet.sp_gera_desconto_avaria (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
p_cod_user,
v_msg,
...
INSERT
   ...
   seq_numer_fat)
VALUES (to_char(v_step),
'vai abrir bilhet.sp_gera_taxa_limpeza_pista',
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
/*
CALL bilhet.sp_gera_taxa_limpeza_pista (p_competencia,
v_seq_numer_prs,
v_seq_numer_fat,
p_cod_user,
v_dat_fim_vig,
...
SET v_tarifa_limpeza_pista = COALESCE(bilhet.fc_tarifa_propt(v_seq_numer_prs, 90, p_competencia), 0);
...
SET v_total_limpeza_pista = (
                              ...
                              FROM
                                ...
                                FROM
                                  ...
                                  bilhet.tbil_relac_visao_percu_padra c
                                WHERE
                                  d.seq_numer_prs = v_seq_numer_prs
                                  AND d.seq_numer_fat IS NULL
                                  AND TRUNC(avb.dhr_embar) <= LAST_DAY(v_dat_fim_vig)
                                  AND avb.seq_numer_avb = d.seq_numer_avb
                                  AND d.seq_numer_prs = c.seq_numer_prs
  ...
  VALUES (v_seq_numer_fat, 90, 'C', (ROUND(v_total_limpeza_pista * v_tarifa_limpeza_pista, 4)), CURRENT TIMESTAMP, p_cod_user);
...
CALL bilhet.sp_gera_juros (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
v_seq_numer_fat_filho,
p_cod_user,
...
CALL bilhet.sp_gera_premio_liquido (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
v_seq_numer_fat_filho,
p_cod_user,
...
CALL bilhet.sp_gera_taxa_agravacao (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
p_cod_user,
v_msg,
...
CALL bilhet.sp_gera_custo_averbacao (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
p_cod_user,
v_msg,
...
CALL bilhet.sp_gera_iof_premio_total (v_dat_fim_vig,
v_seq_numer_prs,
v_seq_numer_fat,
v_seq_numer_fat_filho,
p_cod_user,
...
CALL bilhet.sp_gera_taxa_media (v_seq_numer_fat,
p_cod_user,
v_msg,
v_err_msg);
...
INSERT
   ...
   seq_numer_fat)
VALUES (to_char(v_step),
'vai atualizar bilhet.tbil_relac_averb_propt : v_seq_numer_fat - ' || v_seq_numer_fat,
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
INSERT
   ...
   seq_numer_fat)
VALUES (to_char(v_step),
'vai atualizar bilhet.tbil_fatura : v_seq_numer_fat - ' || v_seq_numer_fat,
CURRENT_TIMESTAMP,
NEXT value FOR bilhet.seq,
v_grupo_seq,
v_seq_gera_fat,
...
INSERT
   INTO    bilhet.tbil_movim_fatura (seq_numer_fat,
   num_tarif,
   des_movim_fat,
   cod_user,
   dhr_alter)
VALUES (v_seq_numer_fat,
81,
...
CALL gedad.sp_dad_atuaz_tarefa_conco(p_ctl_contr_taf,
v_seq_numer_prs,
v_seq_numer_fat,
v_out_result,
v_out_err_message);
