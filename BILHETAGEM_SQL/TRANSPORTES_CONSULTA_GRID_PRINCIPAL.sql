SELECT A.NUM_AVERB AS averbacaoId
    ...
    COALESCE(x.IDT_DIVIS_RCT,'') AS negocios,
    COALESCE(A.NUM_PROPT,0) AS proposta,
    COALESCE(x.NUM_APOLI,0) AS apolice,
    ...
    COALESCE((SELECT DES_MTSIT_AVB FROM MTSIT_AVERBACAO WHERE TIP_MTSIT_AVB = A.TIP_MTSIT_AVB),'') AS motivoCancelamento,
...
FROM APLTRANS.V_AVERBACAO_VEICULO_NEW A 
	LEFT JOIN SEGURO.TSEG_PROPOSTA_SEGURO x ON X.NUM_PROPT = A.NUM_PROPT AND  x.NUM_VERSA_PRS = 1 AND X.TIP_PROPT = 1	 
	INNER JOIN SITUACAO_AVERBACAO S1 ON S1.SIT_AVERB = A.SIT_AVERB
...
WHERE A.CTL_AVERB IN (906900308)
--	AND A.CTL_PESSO = 26494
--  	AND A.DAT_EMBAR BETWEEN '2023-11-01' AND '2023-11-08'
OPTIMIZE FOR 1 ROWS WITH UR  	


SELECT * FROM APLTRANS.V_AVERBACAO_VEICULO_NEW AI 
LEFT JOIN SEGURO.TSEG_PROPOSTA_SEGURO S 
ON S.NUM_PROPT  = AI.NUM_PROPT 
WHERE AI.ctl_averb = 906896152


SELECT * FROM AVERBACAO
WHERE ctl_averb = 906896152
...
/* AVERBA��ES A PARTIR DE MEIA NOITE*/
...
/*INCLUS�O DA TAXA LIMPEZA DE PISTA*/
INSERT INTO bilhet.TBIL_TIPO_TARIFA_PROPOSTA values(90,'071.002.PCE_COBER_LIMP_PST','Taxa Limpeza Pista','P','Pr�mio Limpeza Pista',0.01,CURRENT timestamp)

/* INCLUS�O DA NOVA TAXA PARA O SEGURADO*/
