...
--
      DECLARE v_premio_base DECIMAL(17,
4) DEFAULT 0;
...
--
         ...
         INSERT
   INTO    bilhet.TBIL_VALOR_SOMA_TIPO_TAR_EMBAR (seq_numer_fat,
   cod_cnpj_embarcador,
   nom_embarcador,
   num_tarif,
   idt_premi,
...
FROM    (
   SELECT
       ...
       WHERE            cod_docum_pri = e.cod_cnpj_embarcador) AS nom_embarcador,
       cc1.num_tarif,
   ...
   WHERE        aa1.seq_numer_prs = p_seq_numer_prs
       ...
       --INSTRUÇÃO QDO EMBARCADOR NÃO AGREGA FILIAIS       
   ...
   UNION
       ...
       WHERE            a.seq_numer_prs = p_seq_numer_prs
           ...
           AND TRUNC(avb.dhr_embar) <= LAST_DAY(p_dat_fim_vig)
               ...
               --INSTRUÇÃO QDO EMBARCADOR AGREGA FILIAIS
               AND COALESCE(bb1.sta_agrega_filial,'N') = 'S'
               AND SUBSTR(e.cod_cnpj_embarcador, 1, 8) = SUBSTR(avb.cod_cnpj_tomador, 1, 8)
           GROUP BY
               a.seq_numer_prs,
               e.cod_cnpj_embarcador
       ) x
   ...
   UNION
       ...
       WHERE            d.seq_numer_prs = p_seq_numer_prs
           ...
           AND TRUNC(avb.dhr_embar) <= LAST_DAY(p_dat_fim_vig)
               ...
               --INSTRUÇÃO QDO EMBARCADOR AGREGA FILIAIS
               AND COALESCE(bb1.sta_agrega_filial,'N') = 'S'
               AND SUBSTR(avb.COD_CNPJ_TOMADOR, 1, 8) = SUBSTR(a.cod_cnpj_embarcador, 1, 8)
   )x
GROUP BY
   x.cod_cnpj_embarcador,
   x.nom_embarcador;
...
UNION
   ...
   WHERE        a.seq_numer_prs = p_seq_numer_prs
       ...
       AND COALESCE(bb1.sta_agrega_filial,'N') = 'S'
       AND bb1.NUM_SEQUE_TAR = a.NUM_SEQUE_TAR
       AND SUBSTR(a.cod_cnpj_embarcador, 1, 8) = SUBSTR(b.cod_cnpj_embarcador, 1, 8)
   )x
GROUP BY
   x.cod_cnpj_embarcador,
   x.nom_embarcador,
