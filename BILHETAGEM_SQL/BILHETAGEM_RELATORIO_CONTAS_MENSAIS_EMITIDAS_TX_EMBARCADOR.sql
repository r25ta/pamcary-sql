...
, (
SELECT
	TP.PCE_TARIF_PEC AS taxa
FROM
	APLBIL.BIL_RELAC_VISAO_PERCU_PADRA RP
INNER JOIN APLBIL.BIL_VIGEN_TARIFA_PERCURSO TP ON
	(RP.CTL_VIGEN_PEC = TP.CTL_VIGEN_PEC)
INNER JOIN bilhet.TBIL_RELAC_TARIFA_PERCURSO RTP ON
	RTP.NUM_TARIF_PEC = TP.NUM_TARIF_PEC
WHERE
	RP.SEQ_NUMER_PRS = P.SEQ_NUMER_PRS
	AND RP.SEQ_NUMER_AVB = A.SEQ_NUMER_AVB
UNION 
SELECT
	aa1.vlr_tarif AS taxa
FROM
	APLBIL.v_bil_relac_visao_propt aa1,
	APLBIL.bil_vigencia_taxa_cliente bb1,
	APLBIL.bil_relac_taxa_proposta cc1,
	APLBIL.bil_relac_averb_propt dd1,
	APLBIL.bil_averbacao avb
WHERE
	aa1.seq_numer_prs = P.SEQ_NUMER_PRS
	AND AA1.SEQ_NUMER_AVB = A.SEQ_NUMER_AVB
...
UNION
	SELECT
		aa1.vlr_tarif_prs AS premio
	FROM
		APLBIL.bil_relac_visao_propt aa1,
		APLBIL.bil_vigencia_taxa_cliente bb1,
		APLBIL.bil_relac_taxa_proposta cc1,
		APLBIL.bil_relac_averb_propt dd1,
		APLBIL.bil_averbacao avb
  ...
  FETCH FIRST 1 ROWS ONLY
     ...
     ,(SELECT  CASE WHEN COUNT(ST.SEQ_NUMER_FAT)=0 THEN 'N' ELSE 'S' END TEM_COMERCIAL
           FROM BIL_VALOR_SOMA_TIPO_TARIFA  ST 
...
WHERE 1=1
  ...
  AND FAT.SEQ_NUMER_FAT =7000334044
